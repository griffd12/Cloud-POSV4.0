...
- **LFS First-Run Setup Wizard**: When LFS is unconfigured (`isLfsUnconfigured` flag), the frontend redirects to `/lfs-first-run` — a 4-step wizard (cloud URL → admin auth → property select → save). API endpoints: `/api/lfs/first-run/{validate-cloud,auth,properties,save}`. The save endpoint is guarded (rejects if `LFS_API_KEY` already set) and generates API keys server-side via `crypto.randomBytes`. Page: `client/src/pages/lfs-first-run.tsx`.
