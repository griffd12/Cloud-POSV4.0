...
- **Fresh Install Schema Fix (v3.1.119)**: Backported all migration-only columns into schema.ts CREATE TABLE definitions so fresh installs create correct schemas without needing migrations. Fixed 3 critical gaps: (1) `roles` table missing 4 discount-limit columns (max_item_discount_pct, max_check_discount_pct, max_item_discount_amt, max_check_discount_amt) — caused upsertRole() failure → employee sync failure → white screen, (2) `terminal_sessions` table rewritten to match api.ts/payment-controller usage — added data, created_at, updated_at, cloud_session_id, transaction_type columns, changed amount/tip_amount to TEXT, removed FK constraints — fixed PaymentController crash loop, (3) `check_items` table missing tax_rate_at_sale, tax_mode_at_sale, taxable_amount columns — fixed tax snapshot recording.
- **EMC Live Config Propagation (v3.1.118)**: All EMC config changes (menu items, tenders, discounts, devices, printers, workstations, KDS, POS layouts, employees, roles, etc.) now propagate to CAPS workstations in real-time without restart. Three bugs fixed: (1) `syncDelta()` replaced with `syncFull()` in auto-sync and CONFIG_UPDATE handler — delta endpoint was dead (configVersions table never written to), (2) CAPS now broadcasts `config_update` WebSocket events to connected workstation frontends after sync, triggering React Query cache invalidation and UI refresh. Full propagation chain: EMC → broadcastConfigUpdate → CONFIG_UPDATE WS → CAPS syncFull (50+ entity types) → emitConfigUpdated → broadcastToAll → frontend cache invalidation.
- **CAPS-Cloud Parity (v3.1.117)**: Hierarchical KDS routing (RVC→property→global fallback with workstation device filtering), per-device send mode, station metadata on KDS tickets (stationName/stationType/orderDeviceName/subtotal), PM rollover business date logic, autonomous fiscal scheduler (auto-close/rollover/clock-out), ESCPOSBuilder (cash drawer kick, 3-column formatting, service charge/discount lines, RVC header/trailer), Z-Report and Cashier Report offline endpoints, workstation_order_devices config sync. `datetime('now')` fully replaced with timezone-aware `local_now()` in fiscal-scheduler.
