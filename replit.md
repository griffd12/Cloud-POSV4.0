...
- **Offline Payments (Full Feature Parity)**:
  ...
  - **Settlement Reconciliation**: Cloud-side `/api/lfs/sync/pending-settlements` queries unresolved SAF payments (supports `propertyId` filter for multi-property scoping). `/api/lfs/sync/settle-payment` marks individual payments settled/failed. `/api/lfs/sync/reconcile-saf-batch` settles all pending SAF payments in batch (called automatically during reconnection sync).
